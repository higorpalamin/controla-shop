...
function StockRemoveSearch({ defaultValue = "" }: { defaultValue?: string }) {
  const form = useForm<z.infer<typeof formSchema>>({
    resolver: zodResolver(formSchema),
    defaultValues: {
      produto: defaultValue,
    },
  });
  ...
  const onHandleSubmit = (data: z.infer<typeof formSchema>) => {
    ...
    startTransition(() => {
      if (!search) {
        router.push("/dashboard/stock/remove");
        return;
      }

      router.push(`/dashboard/stock/remove?search=${encodeURIComponent(search)}`);
    });
  };
...
export default StockRemoveSearch;
